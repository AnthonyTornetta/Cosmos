...
impl FluidHolder {
    pub fn new(item: &Item, convert_to: &Item, max_capacity: f32) -> Self {
        Self {
            id: 0,
...
fn register_fluid_holder_items(
    ...
    if let Some(fluid_cell_filled) = items.from_id("cosmos:fluid_cell_filled") {
        fluid_holders.register(FluidHolder::new(fluid_cell_filled, fluid_cell_filled, 10_000.0));
        needs_data.add_item(fluid_cell_filled);

        if let Some(fluid_cell) = items.from_id("cosmos:fluid_cell") {
            fluid_holders.register(FluidHolder::new(fluid_cell, fluid_cell_filled, 10_000.0));
        }
    }
}
