                ...
                if sec - cooldown.last_use_time > cooldown.cooldown_time.as_secs_f32() {
                    ...
                    for line in cannon_system.lines.iter() {
                        if energy_storage_system.get_energy() >= line.property.energy_per_shot {
                            ...
                            let mut direction = line.direction.direction_vec3();

                            // The left/right & up/down directions are inverted for some reason
                            if direction.z == 0.0 {
                                direction *= -1.0;
                            }

                            let laser_velocity = global_transform.affine().matrix3.mul_vec3(direction) * LASER_BASE_VELOCITY;
