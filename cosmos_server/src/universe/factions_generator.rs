    ...
    for ev in evr_generate_system.read() {
        ...
        system.add_item(
            ...
            SystemItem::NpcStation(SystemItemNpcFaction {
                capitol: true,
                faction: fac_id,
                build_type: "cosmos:capitol".into(),
            }),
        );
        ...
        let mut faction_size = rng.random_range(10..15);
        ...
        let buildings = vec!["cosmos:todo"];
        let mut shop_done = false;

        while faction_size != 0 {
            faction_size -= 1;

            let spot = inner_circle.pop().expect("Not enough sectors");
