...
struct MiningLaserBlocks {
    blocks: HashMap<u16, MiningLaserProperty>,
}
...
impl MiningLaserBlocks {
    pub fn insert(&mut self, block: &Block, cannon_property: MiningLaserProperty) {
        self.blocks.insert(block.id(), cannon_property);
    }

    pub fn get(&self, block: &Block) -> Option<&MiningLaserProperty> {
...
pub struct Line {
    ...
    pub property: MiningLaserProperty,
...
impl MiningLaserSystem {
    ...
    fn block_added(&mut self, prop: &MiningLaserProperty, block: &StructureBlock) {
...
fn register_laser_blocks(
    ...
    mut cannon: ResMut<MiningLaserBlocks>,
...
fn block_update_system(
    ...
    laser_cannon_blocks: Res<MiningLaserBlocks>,
    color_blocks: Res<Registry<LaserCannonColorBlock>>,
    blocks: Res<Registry<Block>>,
    mut system_query: Query<&mut MiningLaserSystem>,
...
fn structure_loaded_event(
    ...
    for ev in event_reader.read() {
        if let Ok((structure, mut systems)) = structure_query.get_mut(ev.structure_entity) {
            let mut system = MiningLaserSystem::default();
...
pub(super) fn register<T: States + Clone + Copy>(app: &mut App, post_loading_state: T, playing_state: T) {
    ...
    app.insert_resource(MiningLaserBlocks::default())
        ...
        .register_type::<MiningLaserSystem>();
}
