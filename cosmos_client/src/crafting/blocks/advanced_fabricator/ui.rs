...
use super::{FabricatorMenuSet, OpenAdvancedFabricatorMenu};
...
fn populate_menu(
    ...
    for (ent, fab_menu) in q_added_menu.iter() {
        ...
        let Ok(structure) = q_structure.get(fab_menu.0.structure()) else {
            error!("No structure for advanced_fabricator!");
            continue;
        };

        let Some(inventory) = structure.query_block_data(fab_menu.0.coords(), &q_inventory) else {
            error!("No inventory in advanced_fabricator!");
            continue;
        };
        ...
        ecmds.insert((
            ...
            GuiWindow {
                title: "Advanced Fabricator".into(),
                body_styles: Node {
...
fn listen_create(
    ...
    q_selected_recipe: Query<&Recipe, With<SelectedRecipe>>,
    mut nevw_craft_event: NettyMessageWriter<CraftAdvancedFabricatorRecipeEvent>,
...
pub(super) fn register(app: &mut App) {
    app.add_systems(
        ...
        (
            ...
            color_fabricate_button.chain().in_set(UiSystemSet::DoUi),
        )
            .chain()
