...
use cosmos_core::{
    entities::player::Player, persistence::CustomUnloadDistance, physics::location::Location,
};

use crate::netty::flags::LocalPlayer;

fn unload_far_entities(
    query: Query<(Entity, &Location, &CustomUnloadDistance), (Without<Player>, Without<Parent>)>,
    my_loc: Query<&Location, With<LocalPlayer>>,
    mut commands: Commands,
) {
    if let Ok(my_loc) = my_loc.get_single() {
        for (ent, loc, unload_distance) in query.iter() {
            let ul_distance = unload_distance.block_distance_squared();
