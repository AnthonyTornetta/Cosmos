use cosmos_core::{
    ...
    registry::Registry,
    structure::{block_storage::BlockStorer, chunk::CHUNK_DIMENSIONS, lod_chunk::LodChunk},
};
...
impl<'a> ChunkRendererBackend<LodChunk> for LodChunkRenderingChecker<'a> {
    ...
    fn check_should_render(
        ...
        check_block_at(chunk, check_coords)
    }
}

#[inline]
pub fn check_block_at<C: BlockStorer>(chunk: &C, check_coords: ChunkBlockCoordinate) -> bool {
