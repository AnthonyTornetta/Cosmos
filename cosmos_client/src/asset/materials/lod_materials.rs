...
#[reflect(Default, Debug)]
pub struct LodArrayTextureMaterial {
    ...
    /// The UV channel to use for the [`LodArrayTextureMaterial::base_color_texture`].
    ///
    ...
    /// The UV channel to use for the [`LodArrayTextureMaterial::emissive_texture`].
    ///
    /// Defaults to [`UvChannel::Uv0`].
    ...
    /// The UV channel to use for the [`LodArrayTextureMaterial::metallic_roughness_texture`].
    ///
    /// Defaults to [`UvChannel::Uv0`].
    ...
    ///
    /// For specular transmission usecases with refraction (e.g. glass) use the [`LodArrayTextureMaterial::specular_transmission`] and
    /// [`LodArrayTextureMaterial::ior`] properties instead.
    ///
    ...
    /// ## Notes
    ///
    /// - The material's [`LodArrayTextureMaterial::base_color`] also modulates the transmitted light;
    ...
    /// The material's [`LodArrayTextureMaterial::base_color`] also modulates the transmitted light.
    ///
    /// **Note:** Typically used in conjunction with [`LodArrayTextureMaterial::thickness`], [`LodArrayTextureMaterial::ior`] and [`LodArrayTextureMaterial::perceptual_roughness`].
    ///
    /// ## Performance
    ///
    ...
    /// - If purely diffuse light transmission is needed, (i.e. “translucency”) consider using [`LodArrayTextureMaterial::diffuse_transmission`] instead,
    ...
    ///
    /// **Note:** Typically used in conjunction with [`LodArrayTextureMaterial::specular_transmission`] and [`LodArrayTextureMaterial::ior`], or with
    /// [`LodArrayTextureMaterial::diffuse_transmission`].
    #[doc(alias = "volume")]
    ...
    /// | Moissanite      | 2.65                 |
    ///
    /// **Note:** Typically used in conjunction with [`LodArrayTextureMaterial::specular_transmission`] and [`LodArrayTextureMaterial::thickness`].
    ...
    /// - [`LodArrayTextureMaterial::attenuation_color`];
    /// - [`LodArrayTextureMaterial::thickness`];
    /// - [`LodArrayTextureMaterial::diffuse_transmission`] or [`LodArrayTextureMaterial::specular_transmission`].
    ...
    ///
    /// **Note:** To have any effect, must be used in conjunction with:
    /// - [`LodArrayTextureMaterial::attenuation_distance`];
    /// - [`LodArrayTextureMaterial::thickness`];
    /// - [`LodArrayTextureMaterial::diffuse_transmission`] or [`LodArrayTextureMaterial::specular_transmission`].
    ...
    /// The UV channel to use for the [`LodArrayTextureMaterial::normal_map_texture`].
    ///
    /// Defaults to [`UvChannel::Uv0`].
    ...
    /// # Notes
    ///
    /// Normal mapping with `LodArrayTextureMaterial` and the core bevy PBR shaders requires:
    ...
    /// The UV channel to use for the [`LodArrayTextureMaterial::occlusion_texture`].
    ///
    /// Defaults to [`UvChannel::Uv0`].
    ...
    /// the same way as the [`LodArrayTextureMaterial::perceptual_roughness`].
    ///
    /// If the [`LodArrayTextureMaterial::clearcoat`] value if zero, this has no
    /// effect.
    ///
    /// Defaults to 0.5.
    ...
    /// [`LodArrayTextureMaterial::anisotropy_rotation`] of 0) aligns with the
    ...
    /// the [`LodArrayTextureMaterial::anisotropy_rotation`] parameter.
    ///
    ...
    /// [`LodArrayTextureMaterial::anisotropy_strength`]. The rotation is applied in
    ...
    /// [`LodArrayTextureMaterial::anisotropy_strength`] is zero. Its value can
    /// optionally be adjusted across the mesh with the
    /// [`LodArrayTextureMaterial::anisotropy_texture`].
    ///
    /// See the [`KHR_materials_anisotropy` specification] for more details.
    ///
    /// [`KHR_materials_anisotropy` specification]:
...
impl Default for LodArrayTextureMaterial {
    fn default() -> Self {
...
impl From<Color> for LodArrayTextureMaterial {
...
impl From<Handle<Image>> for LodArrayTextureMaterial {
...
bitflags::bitflags! {
    ...
    /// This is accessible in the shader in the [`LodArrayTextureMaterialUniform`]
    #[repr(transparent)]
    pub struct LodArrayTextureMaterialFlags: u32 {
        /// From [`bevy::prelude::StandardMaterial`]
...
impl LodArrayTextureMaterialFlags {
    const ALPHA_MODE_MASK_BITS: u32 = 0b111;
    const ALPHA_MODE_SHIFT_BITS: u32 = 32 - Self::ALPHA_MODE_MASK_BITS.count_ones();
}

/// The GPU representation of the uniform data of a [`LodArrayTextureMaterial`].
...
pub struct ArrayTextureMaterialUniform {
    ...
    /// The [`LodArrayTextureMaterialFlags`] accessible in the `wgsl` shader.
    pub flags: u32,
    /// When the alpha mode mask flag is set, any base color alpha above this cutoff means fully opaque,
    /// and any below means fully transparent.
    pub alpha_cutoff: f32,
    /// The depth of the [`LodArrayTextureMaterial::depth_map`] to apply.
    pub parallax_depth_scale: f32,
...
impl AsBindGroupShaderType<ArrayTextureMaterialUniform> for LodArrayTextureMaterial {
    fn as_bind_group_shader_type(&self, images: &RenderAssets<GpuImage>) -> ArrayTextureMaterialUniform {
        let mut flags = LodArrayTextureMaterialFlags::NONE;
        if self.base_color_texture.is_some() {
            flags |= LodArrayTextureMaterialFlags::BASE_COLOR_TEXTURE;
        }
        if self.emissive_texture.is_some() {
            flags |= LodArrayTextureMaterialFlags::EMISSIVE_TEXTURE;
        }
        if self.metallic_roughness_texture.is_some() {
            flags |= LodArrayTextureMaterialFlags::METALLIC_ROUGHNESS_TEXTURE;
        }
        if self.occlusion_texture.is_some() {
            flags |= LodArrayTextureMaterialFlags::OCCLUSION_TEXTURE;
        }
        if self.double_sided {
            flags |= LodArrayTextureMaterialFlags::DOUBLE_SIDED;
        }
        if self.unlit {
            flags |= LodArrayTextureMaterialFlags::UNLIT;
        }
        if self.fog_enabled {
            flags |= LodArrayTextureMaterialFlags::FOG_ENABLED;
        }
        if self.depth_map.is_some() {
            flags |= LodArrayTextureMaterialFlags::DEPTH_MAP;
        }
            ...
            if let Some(texture) = images.get(normal_map_id) {
                match texture.texture_format {
                    ...
                    TextureFormat::Rg8Unorm | TextureFormat::Rg16Unorm | TextureFormat::Bc5RgUnorm | TextureFormat::EacRg11Unorm => {
                        flags |= LodArrayTextureMaterialFlags::TWO_COMPONENT_NORMAL_MAP;
                    }
                    _ => {}
                }
            }
            if self.flip_normal_map_y {
                flags |= LodArrayTextureMaterialFlags::FLIP_NORMAL_MAP_Y;
            }
        }
        // NOTE: 0.5 is from the glTF default - do we want this?
        let mut alpha_cutoff = 0.5;
        match self.alpha_mode {
            AlphaMode::Opaque => flags |= LodArrayTextureMaterialFlags::ALPHA_MODE_OPAQUE,
            AlphaMode::Mask(c) => {
                alpha_cutoff = c;
                flags |= LodArrayTextureMaterialFlags::ALPHA_MODE_MASK;
            }
            AlphaMode::Blend => flags |= LodArrayTextureMaterialFlags::ALPHA_MODE_BLEND,
            AlphaMode::Premultiplied => flags |= LodArrayTextureMaterialFlags::ALPHA_MODE_PREMULTIPLIED,
            AlphaMode::Add => flags |= LodArrayTextureMaterialFlags::ALPHA_MODE_ADD,
            AlphaMode::Multiply => flags |= LodArrayTextureMaterialFlags::ALPHA_MODE_MULTIPLY,
            AlphaMode::AlphaToCoverage => {
                flags |= LodArrayTextureMaterialFlags::ALPHA_MODE_ALPHA_TO_COVERAGE;
            }
        };

        if self.attenuation_distance.is_finite() {
            flags |= LodArrayTextureMaterialFlags::ATTENUATION_ENABLED;
        }
...
bitflags! {
    /// The pipeline key for `LodArrayTextureMaterial`, packed into 64 bits.
...
impl From<&LodArrayTextureMaterial> for BlockMaterialKey {
    fn from(material: &LodArrayTextureMaterial) -> Self {
